    ...
    client.sort('q:jobs:active'
      ...
      while (jobs.length) {
        ...
        if (restart) {
          Job.get(id, function(err, job){
            if (err || !job) return;
                // Simply fail the job if it exceeds the specified timeout
                events.emit(job.id, 'failed');
                job.failed().error('watchdog failed job, without attempting to restart, because timeout was exceeded.');
          });
        }
